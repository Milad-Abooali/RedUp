		...
		if ($mmhclass->db->total_rows($sql) < 1) {
			...
			while ($row = $mmhclass->db->fetch_array($sql)) {
				...
				$mmhclass->templ->templ_globals['file_options'] = true;
						...
						$PGAM = basename($_SERVER['PHP_SELF']) . "?page=" . $_GET["page"];
				
				$EXBase64 = EXBase64($imageinfo['filename']);
				$mmhclass->templ->templ_vars[] = array(
					"FILENAME" => $imageinfo['filename'],
					"FILE_TITLE" => mb_strimwidth($imageinfo['file_title'],0,1,'...','utf-8'),
					"FILE_ID" => $imageinfo['file_id'],
					"FILE_DIR" => $imageinfo['dir_name'],
					"PGAM" => $PGAM,
					"BASE64" => $EXBase64,
                    "THUMBNAIL" => 'images/'.$imageinfo['dir_name'].FileThumb($imageinfo['filename']),
