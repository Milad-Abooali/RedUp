<?php
	...
	if ($mmhclass->info->site_installed == false) {
		...
		if (isset($mmhclass->input->cookie_vars['mmh_user_session']) == true && $mmhclass->info->is_robot == false) {
			...
			$sql = $mmhclass->db->query("SELECT * FROM `[1]` WHERE `user_id` = '[2]' AND `session_id` = '[3]' LIMIT 1;", array(MYSQL_USER_SESSIONS_TABLE, $mmhclass->info->user_session['user_id'], $mmhclass->info->user_session['session_id'], $mmhclass->input->server_vars['remote_addr']));
			
			if ($mmhclass->db->total_rows($sql) === 1) {
				$sql = $mmhclass->db->query("SELECT * FROM `[1]` WHERE `user_id` = '[2]' LIMIT 1;", array(MYSQL_USER_INFO_TABLE, $mmhclass->info->user_session['user_id'], $mmhclass->input->server_vars['remote_addr']));
