<?php
	...
	class mmhclass_template_engine
	{
		...
		{
            $EX_page_title = ((isset($this->page_title) == true) ? $this->page_title : $this->mmhclass->info->config['site_name']);
            $secret_key = GenSecretKey($EX_page_title);
			if (isset($this->page_header) == false) {
				$this->templ_vars[] = array(
					...
					"PAGE_TITLE" => $EX_page_title,
					"SECRET_KEY" => $secret_key.'r8b1'.$secret_key,
					"EX_LAST_ICONS" => GetIconsLS(15),
                    "EX_LAST_IMAGES" => GetImagesLS(4),
				);
		...
		function page_footer()
		{
			if ($this->mmhclass->funcs->is_null($this->page_footer) == true) {
				$this->templ_vars[] = array(
                    "TOTAL_UPLOADS" => EXCountAllImages(),
                    "TOTAL_USERS" => EXCountAllUsers(),
                    "RANDOM_IMAGES" => RandomImages(6),
		...
		{ 
          ...
          //  $this->mmhclass->templ->error("Page must be a number!<br /><br /><a href=\"javascript:void(0);\" onclick=\"history.go(-1);\">بازگشت</a>", true);
