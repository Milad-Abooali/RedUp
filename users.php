	...
	switch ($mmhclass->input->get_vars['act']) {
		...
		case "user_list":  case "user-list":
			...
			while ($row = $mmhclass->db->fetch_array($sql)) {
				...
				$mmhclass->templ->templ_vars[] = array(
					...
					"TOTAL_UPLOADS" => $mmhclass->funcs->format_number($mmhclass->db->total_rows($mmhclass->db->query("SELECT * FROM `[1]` WHERE `gallery_id` = '[2]' AND `is_private` = '0';",
					 array(MYSQL_FILE_STORAGE_TABLE, $row['user_id'])))),
				);
		...
		case "gallery": case "my-gallery":
			...
			if ($mmhclass->info->user_owned_gallery == true && $mmhclass->info->is_user == false) {
				...
				$PGAM = basename($_SERVER['PHP_SELF']) . "?act=gallery&page=" . $_GET["page"];
						$mmhclass->templ->templ_vars[] = array(
							"FILE_ID" => $row['file_id'],
		...
		case "albums-c-d":
			...
			$album_title = $mmhclass->input->post_vars['album_title'];
