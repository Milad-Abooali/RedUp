<?php
	...
	// LTE: 1252770406 - Saturday, September 12, 2009, 11:46:46 AM EDT -0400
	// ======================================== /
	...
	if ($mmhclass->funcs->is_null($mmhclass->input->get_vars['file']) == true) {
		...
		$original_filename = (($mmhclass->funcs->is_null($file_info['logs']['original_filename']) == false) ? $file_info['logs']['original_filename'] : $filename);
				...
				$imageinfo = $mmhclass->db->fetch_array($mmhclass->db->query("SELECT * FROM `mmh_file_storage` WHERE `filename` = '$filename' LIMIT 1;", array(MYSQL_FILE_STORAGE_TABLE, $row['filename'])));
		
		$mmhclass->templ->page_title = sprintf($mmhclass->lang['001'], $imageinfo['file_title']);
		...
		$mmhclass->templ->templ_vars[] = array(
			 ...
			 "DATE_UPLOADED" => date('Y-m-d', $file_info['mtime']),
